...
> extends ToneAudioNode<Options> {
	 ...
	 */
	_syncedStart: (time: Seconds, offset: Seconds) => void = noOp;
	_syncedStop: (time: Seconds) => void = noOp;
	...
	sync(): this {
		if (!this._synced) {
			...
			this._syncedStart = (time, offset) => {
				if (offset > 0) {
					...
					if (
						...
						console.log("inside synced start: offset:", stateEvent);
			...
			this._syncedStop = (time) => {
				...
				console.log(
					`value at 200s: ${this._state.getValueAtTime(
						this.context.transport.getSecondsAtTime(200)
					)}`
				);

				if (valueAtTime === "started") {
					this._stop(time);
				}
			};
